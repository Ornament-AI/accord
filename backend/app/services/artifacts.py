...
async def resume_artifact_upload(
    ...
    *,
    ...
    keeps a single storage object across job retries (M-data-12). The row's
    stored checksum is passed as ``expected_checksum_sha256``: if regenerated
    bytes diverge from the original attempt, ``storage.put`` raises
    ``ChecksumMismatch`` instead of silently recording the wrong digest.
    """
    try:
        meta = await storage.put(
            artifact.object_key,
        ...
        # Regenerated bytes diverged from the original attempt — propagate so
        # the job's last_error names the real fault (still retryable).
        raise
    except ObjectStorageError as exc:
        raise ArtifactStorageError(
            "Object storage upload failed; artifact left pending for reconciliation.",
