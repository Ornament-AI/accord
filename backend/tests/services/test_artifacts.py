...
@pytest.mark.asyncio
async def test_resume_pending_artifact_adopts_regenerated_checksum(session):
    """A ``pending`` row has no confirmed object; regenerated bytes may
    legitimately differ (xlsx/zip embed timestamps). The retry adopts the new
    checksum instead of dead-locking on the stale recorded digest."""
    from app.services.artifacts import resume_artifact_upload
    ...
    artifact = ExportArtifact(
        ...
        template_version="v2",
        object_key=f"{world['org_id']}/{uuid4()}",
        checksum_sha256=hashlib.sha256(b"first-attempt-bytes").hexdigest(),
        content_type="application/pdf",
        size_bytes=20,
        status="pending",
        requested_by=world["user_id"],
    ...
    content = b"second-attempt-bytes-differ"
    await _bind(session, world["org_id"], world["user_id"])
    row = await resume_artifact_upload(
        session,
        storage,
        artifact=artifact,
        content=content,
    ...
    assert await storage.get(row.object_key) == content


@pytest.mark.asyncio
async def test_resume_uploaded_artifact_skips_reput_when_object_exists(session):
    """An ``uploaded`` row already has a confirmed object under the recorded
    checksum; a crashed finalize must not rewrite bytes that regenerated
    differently — just finalize the row."""
    from app.services.artifacts import resume_artifact_upload

    world = await _seed_world(session)
    storage = InMemoryObjectStorage()

    original = b"confirmed-upload-bytes"
    artifact = ExportArtifact(
        organization_id=world["org_id"],
        report_type="payslip",
        template_version="v2",
    ...
    row = await resume_artifact_upload(
        ...
        storage,
        artifact=artifact,
        content=b"regenerated-different-bytes",
        content_type="application/pdf",
    )
    assert row.status == "finalized"
    # The confirmed object (and its recorded checksum) wins over regenerated bytes.
    assert row.checksum_sha256 == hashlib.sha256(original).hexdigest()
    assert await storage.get(row.object_key) == original
