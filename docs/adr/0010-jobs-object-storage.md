...
| Column | Type | Purpose |
| --- | --- | --- |
| `id` | `uuid` PK | Stable job id; available to handler results and audit metadata. `export_artifacts` does not currently store it. |
| `organization_id` | `uuid` NOT NULL | Tenant scope; RLS via `app.organization_id`. |
| `job_type` | `text` NOT NULL | Handler key. The current registry contains `noop`, `generate_report`, and `consolidated_xlsx`. Storage-maintenance and explicit lease-reaper handlers are not registered. |
...
The worker loop defaults to a **60s** lease, configurable once for that worker
instance. All job types claimed by the loop use the same value. While
`running`, the worker extends the lease on a cadence under the window (the
default heartbeat cadence is 20s):
...
2. **Per-request authorization** — each download re-checks org membership,
   capabilities, org-scoped artifact lookup, and the artifact `status` and
...
```text
1) Buffer and hash generated bytes; INSERT export_artifacts status=pending
2) Upload object bytes to S3/MinIO at object_key
3) Mark uploaded, then finalize: status=finalized + verified checksum/size
   and object_version
```
...
| 3 Finalize | Checksum mismatch | Do not mark `finalized`; delete/overwrite object; fail attempt. |

Never mark a row `finalized` without a verified SHA-256 and size. Download
handlers ignore non-`finalized` rows and reject rows whose retention deadline
has passed.

The callable `reconcile_orphans()` service currently examines only `pending`
rows older than a cutoff. If the referenced object exists, it reads and hashes
the bytes and finalizes the row; if the object is absent, it marks the row
`deleted`. It does **not** scan the bucket for objects without rows, verify
already-finalized rows, schedule regeneration, or register itself as a worker
job. Those broader reconciliation and scheduling behaviors remain operational
follow-ups.
...
| `posted_run_id` | `uuid` NULL | Posted payroll run this artifact belongs to; nullable for artifacts outside a posted-run export. |
| `report_type` | `text` NOT NULL | Catalog key from [report-catalog.md](../report-specs/report-catalog.md). |
| `variant_key` | `text` NULL | Optional report-family variant such as a component code. |
| `template_version` | `text` NOT NULL | Report template version used for generation. |
| `engine_version` | `text` NULL | Generator/engine build version for reproducibility. |
| `requested_by` | `uuid` NOT NULL | User who requested generation. |
| `created_at` | `timestamptz` NOT NULL DEFAULT `now()` | Intent row creation time. |
| `updated_at` | `timestamptz` NOT NULL | Latest metadata/status update. |
| `checksum_sha256` | `text` NOT NULL | Hex SHA-256 calculated before the pending row is inserted and verified on upload. |
...
| `size_bytes` | `bigint` NOT NULL | Buffered object size, verified against storage metadata. |
| `retention_expires_at` | `timestamptz` NULL | When a finalized artifact becomes ineligible for download. |
| `status` | `text` NOT NULL | Closed set: `pending`, `uploaded`, `finalized`, `expired`, `deleted`. |

**Constraint:** `object_key` is globally unique. RLS continues to scope ordinary
queries by `organization_id`.

**Retention expiry:** `expire_artifacts()` selects `finalized` rows with
`retention_expires_at <= now()` and marks them `expired`. Downloads of
non-`finalized` or expired rows fail before streaming. The current service has
the `deleted` terminal status in its schema, but object deletion/purge
scheduling remains an operational follow-up.
