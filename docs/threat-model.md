...
STRIDE-oriented threat model for Accord (multi-tenant payroll SaaS:
...
**Roles in scope:** Organization administrator, Payroll preparer,
Payroll reviewer, Payroll approver, Payment/report releaser, Auditor,
Platform support administrator.

**Commands in scope:** `calculate`, `submit`, `withdraw`, `approve`,
`reject`, `post`, `reverse`.

**Proven-by** paths are verbatim suite paths from [testing.md](testing.md).
...
| Control | Meaning |
| --- | --- |
| forced RLS (`ALTER TABLE ... FORCE ROW LEVEL SECURITY`) | Tenant tables enable and force RLS so owners cannot skip policies |
| NOSUPERUSER NOBYPASSRLS runtime database role | `accord_app` / `accord_worker` never bypass RLS |
| SET LOCAL per-request tenant context | Transaction-local GUCs (`app.organization_id`, etc.) |
| HTTP-only + Secure + SameSite=Lax cookies (justify Lax for WorkOS redirect auth) | Opaque `accord_session` cookie flags |
| WorkOS webhook signature verification | Reject forged/replayed provider callbacks |
| idempotency keys | Deduplicate state-changing commands per org |
| append-only audit log tables | `audit_events` never updated/deleted in place |
| immutability triggers | Block UPDATE/DELETE on posted `payroll_run_version` |
| capability/permission checks at the API layer | Role/capability enforcement before commands |
| masked fields with a separate audited "reveal" action/endpoint | Default mask; explicit reveal audited |

**CSRF strategy (normative):** SameSite=Lax session cookie **plus**
synchronizer CSRF token on state-changing routes (header must match
server-issued token). Lax covers cross-site cookie sends on top-level
navigations needed for WorkOS redirects; synchronizer tokens defend
state-changing POSTs/PUTs/PATCHes/DELETEs against CSRF including
...
| Field | Detail |
| --- | --- |
| **Vector** | Attacker fixes a pre-auth session id, steals `accord_session` via XSS/network, or reuses a stale cookie after org switch / logout. |
| **Impact** | Full account takeover within the victim’s organization memberships; unauthorized payroll commands. |
| **Mitigations** | HTTP-only + Secure + SameSite=Lax cookies (justify Lax for WorkOS redirect auth); opaque `accord_session` (not JWT in cookie); rotate session id on login and organization switch; short idle/absolute expiry; no session tokens in logs; capability/permission checks at the API layer on every command. |
...
| **Mitigations** | SameSite=Lax session cookie **plus** synchronizer CSRF token on state-changing routes (header must match server-issued token). Lax alone is insufficient for state-changing APIs; synchronizer token is mandatory for mutations. GET/safe navigations remain CSRF-safe by design. |
...
| **Mitigations** | WorkOS webhook signature verification; timestamp/skew checks; idempotent webhook processing with idempotency keys / event-id dedupe; append-only audit log tables for membership mutations. |
...
| Field | Detail |
| --- | --- |
| **Vector** | Client supplies another org’s resource UUID; missing `WHERE organization_id = …`; worker job without `SET LOCAL`; object key enumeration across tenants. |
| **Impact** | Cross-municipality data breach (employees, bank details, payroll results) — breach-class failure. |
| **Mitigations** | forced RLS (`ALTER TABLE ... FORCE ROW LEVEL SECURITY`); NOSUPERUSER NOBYPASSRLS runtime database role (`accord_app` / `accord_worker`); SET LOCAL per-request tenant context from session `active_organization_id` (clients do not choose org via path/body for ordinary resources); capability/permission checks at the API layer; S3 keys scoped `{organization_id}/{object_uuid}`. |
| **Proven-by** | `backend/tests/rls/test_cross_tenant_isolation.py`; `backend/tests/rls/test_forced_rls_coverage.py`; `backend/tests/storage/test_tenant_object_isolation.py`; `backend/tests/api/` |
...
| **Mitigations** | capability/permission checks at the API layer for Organization administrator, Payroll preparer, Payroll reviewer, Payroll approver, Payment/report releaser, Auditor; server-side role from membership tables (not client claims); append-only audit log tables. |
...
| **Mitigations** | Maker/checker workflow enforced in domain + API; distinct capabilities for preparer vs approver; reject self-approval; locking where needed; idempotency keys on commands; append-only audit log tables recording actor, command, and transition. |
...
| **Mitigations** | Immutable posted `payroll_run_version`; immutability triggers; API rejects mutate-in-place (corrections via `reverse` + new version); NOSUPERUSER NOBYPASSRLS runtime database role; append-only audit log tables; content hash on posted results. |
| **Proven-by** | `backend/tests/workflow/test_posted_immutability.py`; `backend/tests/security/test_posted_sql_immutability.py` |
...
| **Mitigations** | Opaque `{organization_id}/{object_uuid}` keys; authorization on artifact metadata under forced RLS; SET LOCAL per-request tenant context before metadata reads; short-lived signed URLs; download events in append-only audit log tables; no PII in object paths. |
...
| **Mitigations** | Dependency/container scanning + SBOM in CI (pip-audit/npm audit, image scan, syft/cyclonedx); pin lockfiles; block critical vulns vs advisory for lower severity ([security.md](security.md)); least-privilege runtime roles. |
...
| **Impact** | Bulk cross-tenant disclosure; long-lived credential reuse. |
...
- Customer endpoint compromise (endpoint malware on clerk workstations) beyond session hardening and CSRF.
- Cryptanalytic breaks of TLS or WorkOS itself (assume current industry primitives).
